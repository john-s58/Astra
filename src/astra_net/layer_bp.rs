...
impl Layer for LayerDense {
    fn feed_forward(&mut self, inputs: &Array1<f64>) -> Array1<f64> {
        ...
        self.output = Some(inputs.dot(&self.weights) + self.biases.clone());
        ...
        println!("INPUTS {:#?}", inputs);
        println!(
            "FEED FORWARD LAYER OUTPUT PRE ACTIVATION {:#?} \n\n",
    ...
    fn back_propagation(&mut self, error: Array1<f64>, learning_rate: f64) -> Array1<f64> {
        ...
        let input_mat = Array2::from_shape_vec(
            (1, self.input.as_ref().unwrap().len()),
            self.input.as_ref().unwrap().to_vec(),
        )
        .unwrap();
